  ...
  const getActiveFields = (): PersonalInfoField[] => {
    ...
    if (govtService?.personal_info_fields && govtService.personal_info_fields.length > 0) {
      ...
      return govtService.personal_info_fields.map(f => ({
        ...
        options: f.options ? f.options.map(o => typeof o === 'string'
          ? { value: o.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || o, label: o }
          : { value: o.value, label: o.label, label_urdu: (o as { label_urdu?: string }).label_urdu }
        ) : undefined,
